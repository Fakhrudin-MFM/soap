...
function SoapClient(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.get(
        ...
        function (err, response, body) {
          if (err) {
            if (options.log) {
              options.log.warn(__('Не удалось получить xml-схему по адресу %schemaUrl.', {schemaUrl}));
            }
            return resolve([]);
          }
          if (response.statusCode === 200) {
            ...
            if (options.log) {
              options.log.warn(__('Не удалось получить xml-схему по адресу %schemaUrl. Код ответа %code', {schemaUrl, code: response.statusCode}));
            }
            resolve([]);
          }
        }
  ...
  function parseWsdl(descriptor, mapping) {
    return new Promise((resolve, reject) => {
      ...
      if (options.log) {
        options.log.info(__('Загружается клиент сервиса для адреса %wsdl', {wsdl}));
      }
          ...
          if (response.statusCode === 200) {
            try {
              ...
              if (services.length > 0) {
                    ...
                    try {
                      ...
                      for (let i = 0; i < services.length; i++) {
                        ...
                        if (portType.length > 0) {
                          ...
                          if (options.log) {
                            options.log.info(__('Инициализирован клиент сервиса %name для адреса %wsdl', {wsdl, name: service.getAttribute('name')}));
                          }
                        } else {
                          return reject(new IonError(Errors.NO_WSDL_INTERFACE));
                        }
                      }
                      return resolve(meta);
                    } catch (err) {
                ...
                if (options.log) {
                  options.log.warn(__('WSDL %wsdl не содержит описания сервисов.', {wsdl}));
                }
            ...
            if (options.log) {
              options.log.warn(__('Не удалось получить описание сервиса по адресу %wsdl. Код ответа %code', {wsdl, code: response.statusCode}));
            }
            clientEvents.emit('failConnect', descriptor, mapping);
            return resolve();
          }
      ...
      .catch((err) => {
        if (options.interval) {
          if (options.log) {
            ...
            options.log.warn(__('Не удалось подключиться по адресу %wsdl [%err]', {wsdl, err: err.message}));
          }
          clientEvents.emit('failConnect', descriptor, mapping);
        } else {
          throw err;
  ...
  if (options.interval) {
    ...
    clientEvents.on('failConnect', (descriptor, mapping) => {
      ...
      if (attempt) {
        setTimeout(() => {
          if (options.log) {
            options.log.info(__('Повторная загрузка (%attempt) клиента сервиса для адреса %wsdl', {wsdl, attempt: attempts[wsdl]}));
          }
